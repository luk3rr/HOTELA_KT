...
sealed class HotelaException(
    ...
    class ExampleNotFoundException(id: UUID) :
        HotelaException("000", "Example with id $id not found")

    @ResponseStatus(HttpStatus.CONFLICT)
    class ExampleAlreadyExistsException(id: UUID) :
        HotelaException("001", "Example with id $id already exists")

    // 1xx - Authentication
    @ResponseStatus(HttpStatus.CONFLICT)
    class EmailAlreadyRegisteredException :
        HotelaException("100", "Email already registered")

    @ResponseStatus(HttpStatus.UNAUTHORIZED)
    class InvalidCredentialsException :
        HotelaException("101", "Invalid credentials")

    @ResponseStatus(HttpStatus.UNAUTHORIZED)
    class InvalidOrExpiredTokenException :
        HotelaException("102", "Token is invalid or expired")
    ...
    // 3xx - Partner
    @ResponseStatus(HttpStatus.NOT_FOUND)
    class PartnerNotFoundException(id: UUID) :
        HotelaException("300", "Partner with id $id not found")

    // 4xx - Hotel
    @ResponseStatus(HttpStatus.NOT_FOUND)
    class HotelNotFoundException(id: UUID) :
        HotelaException("400", "Hotel with id $id not found")

    // 5xx - Room
    @ResponseStatus(HttpStatus.NOT_FOUND)
    class RoomNotFoundException(id: UUID) :
    ...
    @ResponseStatus(HttpStatus.CONFLICT)
    class BookingAlreadyCancelledException(id: UUID) :
    ...
    class InvalidDataException :
        HotelaException("900", "Invalid data provided")
}
